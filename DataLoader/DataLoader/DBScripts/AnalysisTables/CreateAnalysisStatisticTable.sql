...
IF object_id('StagingAnalysisStatistic') is null
Begin
	CREATE TABLE StagingAnalysisStatistic (
		Id              INT           NOT NULL    IDENTITY    PRIMARY KEY,
		[MethodName]        Varchar(100)          NOT NUll,
		SymbolId   INT           NOT NUll,
		FOREIGN KEY (SymbolId) REFERENCES [AllStockSymbols](Id),
		...
		Last10YearValid Decimal(18,3) NOT NULL,
		Last10YearQualified Decimal(18,3) NOT NULL,
		CONSTRAINT StagingAnalysisStatistic_Method_Symbol UNIQUE ([MethodName], SymbolId)
	);

	CREATE INDEX symbolIndex
	ON StagingAnalysisStatistic (SymbolId);
End